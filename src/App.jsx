    ...
    >
        ...
        <aside className="w-64 shrink-0 border-r border-white/5 bg-black/10 p-6 flex flex-col gap-8 hidden md:flex">
          ...
          <div className="mt-auto p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20">
            ...
            <p className="text-[10px] text-slate-400 opacity-80 leading-relaxed">
              Network protection bypassed. All 9 games are fully unlocked and ready to play.
            </p>
          </div>
        </aside>
        ...
        <main className="flex-1 p-4 sm:p-8 lg:p-10 flex flex-col overflow-y-auto">
          ...
          {selectedGame ? (
            ...
            <div className="w-full flex flex-col items-center max-w-5xl mx-auto">
              ...
              {/* Render Selected Game inside Immersive Glow Container */}
              <div className="w-full flex justify-center border border-white/10 bg-black/40 backdrop-blur-md rounded-2xl p-4 sm:p-6 shadow-2xl">
                {renderActiveGame()}
              </div>

              {/* Quick Game Switcher Bottom Bar */}
              <div className="w-full max-w-3xl mt-8 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
                <span className="font-bold text-xs uppercase tracking-widest text-slate-400">Quick Launch:</span>
                <div className="flex flex-wrap gap-2 justify-center">
                  {GAMES_LIST.filter((g) => g.id !== selectedGame.id)
                    .slice(0, 6)
                    .map((g) => (
                      <button
                        key={g.id}
                        onClick={() => handlePlayGame(g)}
                        className="px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white border border-white/10 hover:border-indigo-500/50 text-xs transition cursor-pointer"
                      >
                        {g.title}
                      </button>
                    ))}
              ...
              <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 shrink-0 pb-4 border-b border-white/5">
                <div>
                  ...
                  <p className="text-slate-400 text-xs sm:text-sm mt-1">
                    Most played titles across your school network • 100% In-Browser
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-2 bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 hover:border-indigo-500/50 text-slate-300 text-xs flex items-center gap-1.5 transition"
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-12 text-center flex flex-col items-center">
                  ...
                  <p className="text-xs text-slate-400 max-w-sm mb-4">
                    {selectedCategory === 'favorites'
                      ...
                      : `No games match the query "${searchQuery}". Try searching for snake, tetris, or arcade.`}
                  </p>
                  <button
                    onClick={() => {
                      setSearchQuery('');
                    ...
                    className="px-5 py-2 bg-white text-black hover:bg-indigo-400 text-xs font-bold uppercase tracking-wider rounded-full transition"
                  >
                    View All Games
                  </button>
                </div>
      ...
      <footer className="h-12 shrink-0 bg-black flex items-center px-6 sm:px-10 border-t border-white/5 text-[10px] text-slate-500 uppercase tracking-widest justify-between">
        ...
        <div className="flex items-center gap-4">
          <span>
            Status: <span className="text-indigo-400">All Nodes Optimal</span>
          </span>
